...
def make_api_request(n: int):
    ...
    try:
        response = post('https://api.igdb.com/v4/games', **{'headers': {'Client-ID': 'usyz3spa2yg21kg7c7sg0i4dk7l5gb', 'Authorization': 'Bearer wmrl8wh7vzx8xqr2qacspowyf0886c'},'data': 'fields name,rating,rating_count,similar_games,storyline,summary,tags,themes,url,websites; where id>={0} & id < {1}; sort id asc; limit 500;'.format(n,n+500)})
